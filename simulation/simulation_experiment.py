...
def run_simulation(
    ...
    # --- NEW: external push schedule ---
    push_schedule: list[dict] | None = None,  # [{"episode": 0, "start_s": 3.0, "duration_s": 0.2, "force_xyz": [0,300,0], "torque_xyz": [0,0,0]}]
    # --- NEW: per-episode outcome log (success/fail) ---
    episode_outcomes_path: PathLike | None = None,
):
    ...
    # Log a minimal but useful set for analysis/plots
    state_obs_names = (
        "base_pos",
        "base_ori_euler_xyz",
        "base_lin_vel",
    ...
    env = QuadrupedEnv(
        ...
        state_obs_names=tuple(state_obs_names),  # Desired quantities in the 'state' dict
    )
    ...
    # Resolve base body id via the free joint (robust across robots)
    base_body_id = None
    for j in range(env.mjModel.njnt):
        if env.mjModel.jnt_type[j] == mujoco.mjtJoint.mjJNT_FREE:
            base_body_id = int(env.mjModel.jnt_bodyid[j])
            break
    if push_schedule:
        print(f"[push] base_body_id = {base_body_id}")

    # Helper to compute current external wrench from schedule
    def _active_wrench(t_s: float, episode_idx: int):
        if not push_schedule or base_body_id is None:
            return None
        for ev in push_schedule:
            ep_ok = ("episode" not in ev) or (ev["episode"] == episode_idx) or (ev["episode"] == "all")
            if not ep_ok:
                continue
            t0 = float(ev["start_s"])
            dt = float(ev["duration_s"])
            if t0 <= t_s < t0 + dt:
                f = np.array(ev.get("force_xyz", [0, 0, 0]), dtype=float)
                tau_wrench = np.array(ev.get("torque_xyz", [0, 0, 0]), dtype=float)
                return f, tau_wrench
        return None

    # --- NEW: visualization state for push arrow/sphere ---
    push_vec_geom_id = -1
    push_sphere_geom_id = -1
    latest_push_force = np.zeros(3)
    PUSH_LEN_PER_N = 0.01  # arrow length per Newton
    ...
    if recording_path is not None:
        ...
        h5py_writer = H5Writer(
            ...
            # Ensure writer knows which extra obs to store (same as state_obs_names)
            # extra_obs=list(state_obs_names),
        )
    ...
    # Prepare outcomes CSV if requested
    outcomes_writer = None
    outcomes_fh = None
    if episode_outcomes_path is not None:
        outcomes_path = pathlib.Path(episode_outcomes_path)
        outcomes_path.parent.mkdir(parents=True, exist_ok=True)
        new_file = not outcomes_path.exists()
        outcomes_fh = open(outcomes_path, "a", newline="")
        outcomes_writer = csv.writer(outcomes_fh)
        if new_file:
            outcomes_writer.writerow(["timestamp", "episode", "success", "terminated", "truncated"])
    ...
    for episode_num in range(N_EPISODES):
        ...
        for _ in tqdm(range(N_STEPS_PER_EPISODE), desc=f"Ep:{episode_num:d}-steps:", total=N_STEPS_PER_EPISODE):
            ...
            if base_body_id is not None:
                ...
                wr = _active_wrench(env.simulation_time, episode_num)
                if wr is not None:
                    f, wrench_tau = wr
                    env.mjData.xfrc_applied[base_body_id, 0:3] = f
                    env.mjData.xfrc_applied[base_body_id, 3:6] = wrench_tau
                    latest_push_force = f.copy()
                else:
                    latest_push_force[:] = 0.0
            ...
            if render and (time.time() - last_render_time > 1.0 / RENDER_FREQ or env.step_num == 1):
                ...
                # --- NEW: visualize push as an arrow and a small sphere at base ---
                push_norm = float(np.linalg.norm(latest_push_force))
                # alpha fades when inactive
                alpha = 0.85 if push_norm > 1e-3 else 0.10
                # Arrow from base position, direction = force, length proportional to |F|
                push_vec_geom_id = render_vector(
                    viewer=env.viewer,
                    vector=latest_push_force,
                    pos=base_pos,  # world-frame
                    scale=push_norm * PUSH_LEN_PER_N,
                    color=np.array([1.0, 0.0, 0.0, alpha]),
                    geom_id=push_vec_geom_id,
                )
                # Small sphere to denote application point
                push_sphere_geom_id = render_sphere(
                    viewer=env.viewer,
                    position=base_pos.tolist(),
                    diameter=0.05,
                    color=np.array([1.0, 0.3, 0.3, alpha]).tolist(),
                    geom_id=push_sphere_geom_id,
                )    
            ...
            if env.step_num >= N_STEPS_PER_EPISODE or is_terminated or is_truncated:
                # --- NEW: if we started but didn't finish, log failure now ---
                if start_xy is not None and end_xy is not None and fsm_state == "timing" and start_time is not None:
                    fail_end_time = env.simulation_time  # end of episode step
                    dt_fail = float(fail_end_time - start_time)
                    print(f"[timing] Attempt failed (no end within radius) at ep {episode_num}, pass {current_pass_idx}; "
                          f"elapsed {dt_fail:.3f}s")
                    if timing_writer is not None:
                        timing_writer.writerow([
                            datetime.now().isoformat(timespec="seconds"),
                            episode_num, current_pass_idx if current_pass_idx is not None else pass_counter,
                            float(start_time), float(fail_end_time),
                            dt_fail, 0,  # success=0
                            float(start_xy[0]), float(start_xy[1]),
                            float(end_xy[0]), float(end_xy[1]),
                            bool(timing_use_com), float(timing_radius),
                        ...
                        timing_csv_fh.flush()
                        pass_counter += 1
                        current_pass_idx = None
                
                # --- NEW: write per-episode outcome row ---
                if outcomes_writer is not None:
                    success = int(not (is_terminated or is_truncated))
                    outcomes_writer.writerow([
                        datetime.now().isoformat(timespec="seconds"),
                        episode_num, success, int(bool(is_terminated)), int(bool(is_truncated)),
                    ])
                    outcomes_fh.flush()
